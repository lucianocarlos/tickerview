...
@st.dialog("⚖️ Comparação Profunda de Modelos", width="large")
def render_deepcomparison(base_id, base_dataset, compare_id, compare_dataset):
    ...
    df_base = df_global[(df_global["exp_id"] == base_id) & (df_global["dataset_version"] == base_dataset)]
    df_comp = df_global[(df_global["exp_id"] == compare_id) & (df_global["dataset_version"] == compare_dataset)]
    ...
    st.markdown("#### ⚙️ Configurações (Laranja = Diferente)")
    
    configA = row_base.get("experiment_config_dict", {})
    if not isinstance(configA, dict): configA = {}
    paramsA = row_base.get("parameters_dict", {})
    if not isinstance(paramsA, dict): paramsA = {}
    
    configB = row_comp.get("experiment_config_dict", {})
    if not isinstance(configB, dict): configB = {}
    paramsB = row_comp.get("parameters_dict", {})
    if not isinstance(paramsB, dict): paramsB = {}
    ...
    if xai_base and xai_comp:
        ...
        for feat, imp in top_B_15:
            ...
            feature_lines.append(f"**{posB}º** | {feat} | Imp: {abs(imp):.4f} | {status}")
        ...
        st.markdown("*Dados de explicabilidade (XAI) indisponíveis para um dos modelos.*")

    st.divider()

    # ------------------ GRID SEARCH CONFIG (EXPANDER NO FINAL) ------------------
    with st.expander("🔍 Configuração do Grid Search (Drill Down)"):
