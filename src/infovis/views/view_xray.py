...
def render_xray(df_report, exp_id_selecionado):
    ...
    st.markdown(f"<div style='font-size:1.2em; font-weight:bold; margin-bottom: 10px;'>Detalhes de {exp_id_selecionado}</div>", unsafe_allow_html=True)
    ...
    with col_spider:
        ...
        categories = ['Val Acc', 'Val F1', 'Test Acc', 'Test F1', 'Test Prec', 'Test Rec']
        valores = [
    ...
    xai_dict = load_xai_metadata(row['exp_id'])
    
    if xai_dict:
        # Pega as top 10 features ordenadas
        top_features = sorted(xai_dict.items(), key=lambda item: item[1], reverse=True)[:10]
        ...
        fig_xai = px.bar(
            ...
            color=values, color_continuous_scale='teal'
        )
        ...
        st.info("Nenhuma matriz de explicabilidade (XAI) nativa foi encontrada no Datalake para este modelo. Isto ocorre em modelos caixa-preta (ex: Redes Neurais, KNN) ou não-particionados (Logística, Ensembles), ou porque o modelo não foi o Campeão Absoluto de sua bateria local, não tendo seus vetores salvos pelo AutoML.")
